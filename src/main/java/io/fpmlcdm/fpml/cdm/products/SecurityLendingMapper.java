...
/**
 * Security lending (SBL) FpML→CDM mapper.
 *
 * PARTY_1 convention: follows the first party declared in document order (same as CreditDefaultSwapMapper).
 * The reference CDM for available SBL examples is effectively a Trade carrying only
 * tradeIdentifier, tradeDate, parties, partyRole and contractDetails — no product / tradeLot / counterparty.
...
public class SecurityLendingMapper implements ProductMapper {
    ...
    public TradeState map(Document doc, Element trade) {
        ...
        // Trade identifiers — handle both regular and versioned trade IDs (execution notifications).
        ...
        // Build contract details (masterAgreement, creditSupportAgreement, otherAgreement)
