...
public final class InterestRatePayoutMapper {
    ...
    private static cdm.product.common.settlement.PrincipalPayment buildOnePrincipalPayment(
        ...
        com.rosetta.model.lib.records.Date unadjustedDate = DateMapper.parse(unadj);
        if (unadjustedDate != null) dateB.setUnadjustedDate(unadjustedDate);
        if (adj != null) {
    ...
    private static CalculationPeriodDates buildCalcDates(Element fpml) {
        ...
        com.rosetta.model.lib.records.Date firstRegularStart = DateMapper.parse(firstRegStart);
        if (firstRegularStart != null) b.setFirstRegularPeriodStartDate(firstRegularStart);
        String lastRegEnd = XmlUtils.childText(fpml, "lastRegularPeriodEndDate");
        com.rosetta.model.lib.records.Date lastRegularEnd = DateMapper.parse(lastRegEnd);
        if (lastRegularEnd != null) b.setLastRegularPeriodEndDate(lastRegularEnd);
        String firstCompEnd = XmlUtils.childText(fpml, "firstCompoundingPeriodEndDate");
        com.rosetta.model.lib.records.Date firstCompoundingEnd = DateMapper.parse(firstCompEnd);
        if (firstCompoundingEnd != null) b.setFirstCompoundingPeriodEndDate(firstCompoundingEnd);
    ...
    private static PaymentDates buildPaymentDates(Element fpml) {
        ...
        com.rosetta.model.lib.records.Date firstPayment = DateMapper.parse(firstPaymentDate);
        if (firstPayment != null) b.setFirstPaymentDate(firstPayment);
        String lastRegularPayment = XmlUtils.childText(fpml, "lastRegularPaymentDate");
        com.rosetta.model.lib.records.Date lastRegularPaymentDate = DateMapper.parse(lastRegularPayment);
        if (lastRegularPaymentDate != null) b.setLastRegularPaymentDate(lastRegularPaymentDate);
