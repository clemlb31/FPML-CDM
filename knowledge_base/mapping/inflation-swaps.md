# FpML → CDM mapping — Inflation swaps — STUB

> Status: scaffold. Source side: [../fpml/inflation-swaps.md](../fpml/inflation-swaps.md). Read
...
## CDM target
An `InterestRatePayout` with inflation index + lag; reuse the rates mapping then add inflation fields. FX-linked-notional variant uses `quantityMultiplier.fxLinkedNotionalSchedule`.

## How to fill this in
Work a train pair in `data/train/inflation-swaps-*/` (FpML + expected CDM JSON): for each populated CDM
