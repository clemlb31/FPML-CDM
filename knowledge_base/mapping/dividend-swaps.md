# FpML → CDM mapping — Dividend swaps — STUB

> Status: scaffold. Source side: [../fpml/dividend-swaps.md](../fpml/dividend-swaps.md). Read
...
## CDM target
A `PerformancePayout` (dividend return), often with an `InterestRatePayout` financing leg.

## How to fill this in
Work a train pair in `data/train/dividend-swaps-*/` (FpML + expected CDM JSON): for each populated CDM
